...
def get_engine(engine_name: str, **kwargs) -> EngineLM:
    ...
    elif engine_name.startswith("openrouter"):
        from .openai import ChatOpenAI, OPENROUTER_BASE_URL
        # remove the "openrouter-" prefix from the model name
        model_string = engine_name.replace("openrouter-", "")
        return ChatOpenAI(
